...
/**
 * @title BranchHarness
 * @notice Exposes internal library functions for branch coverage testing
 */
contract BranchHarness {
    // ================== FeeCalculator ==================

    function calcTradingFee(
        uint256 size, uint256 makerBps, uint256 takerBps, uint256 minFeeUsdc,
        bool isMaker, uint256 referralDiscountBps
    ) external pure returns (uint256) {
        DataTypes.FeeConfig memory cfg = DataTypes.FeeConfig({
            makerFeeBps: makerBps, takerFeeBps: takerBps, minFeeUsdc: minFeeUsdc,
            lpShareBps: 7000, insuranceShareBps: 2000, treasuryShareBps: 1000
        });
    ...
    function calcOpeningFee(uint256 size, uint256 takerBps, uint256 minFee) external pure returns (uint256) {
        DataTypes.FeeConfig memory cfg = DataTypes.FeeConfig({
            makerFeeBps: 2, takerFeeBps: takerBps, minFeeUsdc: minFee,
            lpShareBps: 7000, insuranceShareBps: 2000, treasuryShareBps: 1000
        });
        return FeeCalculator.calculateOpeningFee(size, cfg);
    }

    function calcClosingFee(uint256 size, uint256 makerBps, uint256 takerBps, uint256 minFee, bool isMarket) external pure returns (uint256) {
        DataTypes.FeeConfig memory cfg = DataTypes.FeeConfig({
            makerFeeBps: makerBps, takerFeeBps: takerBps, minFeeUsdc: minFee,
            lpShareBps: 7000, insuranceShareBps: 2000, treasuryShareBps: 1000
        });
        return FeeCalculator.calculateClosingFee(size, cfg, isMarket);
    }

    function calcLiqFee(uint256 size, uint256 healthFactor) external pure returns (uint256 total, uint256 liqFee, uint256 insFee) {
    ...
    function splitFees(uint256 total, uint256 lpBps, uint256 insBps, uint256 treasBps) external pure returns (uint256, uint256, uint256) {
        DataTypes.FeeConfig memory cfg = DataTypes.FeeConfig({
            makerFeeBps: 2, takerFeeBps: 5, minFeeUsdc: 0,
            lpShareBps: lpBps, insuranceShareBps: insBps, treasuryShareBps: treasBps
        });
        return FeeCalculator.splitFees(total, cfg);
    }

    function validateFeeConfig(uint256 makerBps, uint256 takerBps, uint256 lpBps, uint256 insBps, uint256 treasBps) external pure returns (bool) {
        DataTypes.FeeConfig memory cfg = DataTypes.FeeConfig({
            makerFeeBps: makerBps, takerFeeBps: takerBps, minFeeUsdc: 0,
            lpShareBps: lpBps, insuranceShareBps: insBps, treasuryShareBps: treasBps
        });
        return FeeCalculator.validateFeeConfig(cfg);
    }

    function calcKeeperReward(uint256 gasUsed, uint256 gasPrice, uint256 ethPrice, uint256 multBps) external pure returns (uint256) {
        return FeeCalculator.calculateKeeperReward(gasUsed, gasPrice, ethPrice, multBps);
    }

    function calcGasRefund(uint256 gasUsed, uint256 gasPrice, uint256 ethPrice, uint256 maxRefund) external pure returns (uint256) {
    ...
    function calcFundingIntervals(uint64 lastSettlement, uint256 curTime, uint256 interval) external pure returns (uint256) {
        return PositionMath.calculateFundingIntervals(lastSettlement, curTime, interval);
    }

    function validateSlippage(uint256 expected, uint256 actual, uint256 maxBps, bool isLong) external pure returns (bool) {
    ...
    function isLiquidatable(uint128 size, uint128 entry, uint8 flags, uint64 leverage, uint256 currentPrice, uint256 collateral) external pure returns (bool, uint256) {
        DataTypes.Position memory pos;
        pos.size = size;
    ...
    function isLiquidatableClosed(uint128 size, uint128 entry, uint8 flags, uint256 currentPrice, uint256 collateral) external pure returns (bool, uint256) {
        DataTypes.Position memory pos;
        pos.size = size;
        pos.entryPrice = entry;
        pos.flags = flags;
