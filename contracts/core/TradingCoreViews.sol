...
contract TradingCoreViews is Ownable {
    ...
    function initialize(address _tradingCore, address _vaultCore, address _oracleAggregator) external onlyOwner {
        ...
        if (_tradingCore == address(0) || _vaultCore == address(0) || _oracleAggregator == address(0)) revert ZeroAddress();
    ...
    function getCircuitBreakerStatus(address market) external view returns (
        bool isRestricted,
        uint256 activeBreakers,
        bool globalPause
    ) {
    ...
    function _getProtocolHealthState() internal view returns (bool isHealthy, uint256 totalBadDebt, uint64 lastHealthCheck) {
