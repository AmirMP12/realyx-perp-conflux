...
library TradingLib {
    ...
    function addCollateral(
        ...
        p.liquidationPrice = uint128(PositionMath.calculateLiquidationPrice(
    ...
    function settlePositionFunding(
        ...
        return FundingLib.settlePositionFunding(
            positionId,
            oracleAggregator,
            positions,
            positionCollateral,
    ...
    function settlePositionFundingWithDividends(
        ...
        if (address(dividendManager) != address(0)) {
            ...
            if (bytes(mId).length > 0) {
                (int256 divAmount, uint256 newIndex) = DividendSettlementLib.settleDividends(positionId, p, mId, positionDividendIndex[positionId], dividendManager);
                if (newIndex > positionDividendIndex[positionId]) {
                    positionDividendIndex[positionId] = newIndex;
                    if (divAmount != 0) applyFundingToCollateral(positionCollateral[positionId], -divAmount, positionId);
                }
            }
        }
    }

    function _closePositionUpdateMarketAndFinalize(
        uint256 positionId,
        uint256 closeSize,
        uint256 totalSize,
        ...
        if (closeSize >= totalSize) {
            ...
            unchecked {
                userExposure[posOwner] = userExposure[posOwner] > DataTypes.toUsdcPrecision(closeSize) ? userExposure[posOwner] - DataTypes.toUsdcPrecision(closeSize) : 0;
            }
            ...
            uint256 newLeverage = calculateNewLeverage(uint256(position.size), positionCollateral[positionId].amount);
            position.leverage = uint64(newLeverage);
            position.liquidationPrice = uint128(PositionMath.calculateLiquidationPrice(uint256(position.entryPrice), newLeverage, m.maintenanceMargin, isLong));
        }
    }
    ...
    function executeOrderFull(
        ...
        orderIdOut = orderId;
        isIncrease = (order.orderType == DataTypes.OrderType.MARKET_INCREASE || order.orderType == DataTypes.OrderType.LIMIT_INCREASE);
        ...
        if (order.orderType == DataTypes.OrderType.LIMIT_INCREASE) {
            if ((order.isLong && currentPrice > order.triggerPrice) || (!order.isLong && currentPrice < order.triggerPrice)) revert InvalidOrder();
        }
        if (order.maxSlippage > 0 && order.triggerPrice > 0) {
            uint256 priceDeviation = currentPrice > order.triggerPrice ? ((currentPrice - order.triggerPrice) * BPS) / order.triggerPrice : ((order.triggerPrice - currentPrice) * BPS) / order.triggerPrice;
        ...
        positionId = executeOrder(order, ctx, positions, positionCollateral, markets, userPositions, userExposure, nextPositionId);
        if (isIncrease) {
            if (address(dividendManager) != address(0)) {
    ...
    function executeOrder(
        ...
        uint256 nextPosId
         ...
         if (order.orderType == DataTypes.OrderType.MARKET_INCREASE || order.orderType == DataTypes.OrderType.LIMIT_INCREASE) {
             return _executeIncrease(order, ctx, positions, positionCollateral, markets, userPositions, userExposure, nextPosId);
         }
         if (order.orderType == DataTypes.OrderType.MARKET_DECREASE || order.orderType == DataTypes.OrderType.LIMIT_DECREASE) {
    ...
    function _executeDecrease(
        ...
        uint256 closeSizeInternal = order.sizeDelta > 0 ? DataTypes.toInternalPrecision(order.sizeDelta) : uint256(position.size);
    ...
    function _executeIncrease(
        ...
        if (order.triggerPrice > 0 && (order.orderType == DataTypes.OrderType.MARKET_INCREASE || order.orderType == DataTypes.OrderType.MARKET_DECREASE)) {
        ...
        uint256 borrowAmountUsdc = DataTypes.toUsdcPrecision(internalSize - marginInternal);
        ...
        positions[positionId] = DataTypes.Position({
            ...
            liquidationPrice: uint128(PositionMath.calculateLiquidationPrice(
                currentPrice, 
                leverage, 
                m.maintenanceMargin, 
                order.isLong
    ...
    function liquidatePosition(
        ...
        return LiquidationLib.liquidatePosition(positionId, liqCtx, positions, positionCollateral, markets, userExposure);
    }

    function liquidatePositionWithFailedRepaymentRecord(
        uint256 positionId,
        LiquidatePositionContext memory ctx,
        address oracleAggregator,
        mapping(uint256 => DataTypes.Position) storage positions,
    ...
    ) external returns (uint256 liquidatorReward, bool didRecordFailedRepayment) {
        DataTypes.Position storage position = positions[positionId];
        LiquidationLib.LiquidatePositionContext memory liqCtx = LiquidationLib.LiquidatePositionContext(
            ctx.usdc, ctx.liquidityVault, ctx.oracleAggregator, ctx.positionToken,
            ctx.treasury, ctx.insuranceFund, ctx.liquidationTiers, ctx.liquidationDeviationBps
        );
        liquidatorReward = LiquidationLib.liquidatePosition(positionId, liqCtx, positions, positionCollateral, markets, userExposure);
        if (position.state == DataTypes.PosStatus.LIQUIDATED && failedRepayments[positionId].amount == 0) {
            uint256 repayAmount = DataTypes.toUsdcPrecisionCeil(uint256(position.size) - positionCollateral[positionId].amount);
            if (repayAmount > 0) {
                (uint256 currentPrice,,) = IOracleAggregator(oracleAggregator).getPrice(position.market);
                int256 pnl = PositionMath.calculateUnrealizedPnL(uint256(position.size), uint256(position.entryPrice), currentPrice, DataTypes.isLong(position.flags));
                recordFailedRepayment(positionId, repayAmount, position.market, DataTypes.isLong(position.flags), pnl, failedRepayments, failedRepaymentIds, failedRepaymentIndex);
                didRecordFailedRepayment = true;
            }
        }
    }

    function removeFailedRepaymentId(
        uint256 positionId,
        uint256[] storage failedRepaymentIds,
        mapping(uint256 => uint256) storage failedRepaymentIndex
    ) external {
        ...
        if (idx >= len || failedRepaymentIds[idx] != positionId) return;
    ...
    function closePosition(
        ...
        realizedPnL = PositionCloseLib.closePosition(positionId, closeSize, minReceive, closeCtx, positions, positionCollateral, markets, userExposure);
    ...
    function _updateMarketStats(
        ...
        DataTypes.Market storage m = markets[position.market];
        uint256 cost = (closeSize * uint256(position.entryPrice)) / PRECISION;
        if (DataTypes.isLong(position.flags)) {
            m.totalLongSize = m.totalLongSize > closeSize ? m.totalLongSize - closeSize : 0;
            m.totalLongCost = m.totalLongCost > cost ? m.totalLongCost - cost : 0;
        } else {
            m.totalShortSize = m.totalShortSize > closeSize ? m.totalShortSize - closeSize : 0;
    ...
    function _updatePositionAfterClose(
        ...
        uint256 exposureDecreaseUsdc = DataTypes.toUsdcPrecision(closeSize);
        if (closeSize >= totalSize) {
            position.state = DataTypes.PosStatus.CLOSED;
            userExposure[posOwner] = userExposure[posOwner] > exposureDecreaseUsdc ? userExposure[posOwner] - exposureDecreaseUsdc : 0;
            IPositionToken(positionToken).burn(positionId);
            positionCollateral[positionId].amount = 0;
        } else {
            position.size = uint128(totalSize - closeSize);
            ...
            uint256 newLeverage = calculateNewLeverage(uint256(position.size), positionCollateral[positionId].amount);
            position.leverage = uint64(newLeverage);
            position.liquidationPrice = uint128(PositionMath.calculateLiquidationPrice(uint256(position.entryPrice), newLeverage, m.maintenanceMargin, DataTypes.isLong(position.flags)));
        }
    }
    ...
    function _divCeil(uint256 a, uint256 b) private pure returns (uint256 result) {
        if (b == 0) return 0;
        result = a / b;
        if (a % b != 0) {
            result += 1;
    ...
    function cancelOrder(
        ...
        if ((order.orderType == DataTypes.OrderType.MARKET_INCREASE || order.orderType == DataTypes.OrderType.LIMIT_INCREASE) && order.collateralDelta > 0) {
    ...
    function executeStopLossTakeProfit(
        ...
        for (uint256 i = 0; i < positionIds.length;) {
            ...
            if (pos.state == DataTypes.PosStatus.OPEN) {
                ...
                if (PositionMath.shouldTriggerStopLoss(pos, price) || PositionMath.shouldTriggerTakeProfit(pos, price)) {
                    FundingLib.settlePositionFunding(id, oracleAggregator, positions, positionCollateral, fundingStates, positionCumulativeFunding);
                    if (address(dividendManager) != address(0)) {
                        string memory mId = marketIds[pos.market];
                        if (bytes(mId).length > 0) {
                            (int256 divAmount, uint256 newIndex) = DividendSettlementLib.settleDividends(id, pos, mId, positionDividendIndex[id], dividendManager);
                    ...
                    emit StopLossTakeProfitExecuted(id, price, PositionMath.shouldTriggerStopLoss(pos, price) ? "StopLoss" : "TakeProfit");
                    unchecked { ++executedCount; }
                }
            }
            unchecked { ++i; }
        }
