...
describe("Coverage Maximizer 2 - Uncovered Libraries", function () {
    ...
    describe("FlashLoanCheck Coverage", function () {
        it("should exercise all FlashLoanCheck paths", async function () {
            ...
            await expect(harness.testValidateFlashLoan(user, user, false, 10, 2))
                .to.be.reverted;
            ...
            // Trigger FlashLoanDetected by multiple calls from same user in same block using the double-call harness function
            await expect(harness.testDoubleValidateFlashLoan(env.alice.address, env.alice.address, false, 5, 10))
                .to.be.reverted;

            // Trigger RateLimitExceeded (maxActionsPerBlock = 1)
            await harness.testValidateFlashLoan(charlie, charlie, false, 1, 2);
            await expect(harness.testValidateFlashLoan(env.bob.address, env.bob.address, false, 1, 2))
                .to.be.reverted;
