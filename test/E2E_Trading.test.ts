...
describe("E2E Trading - Full Protocol Lifecycle", function () {
    ...
    it("should execute a full lifecycle: Open -> Update -> Sell -> Liquidate", async function () {
        // sizeDelta = notional value in USDC precision (6 dec), converted internally via toInternalPrecision
        // collateralDelta = in internal precision (18 dec), used as-is
        const orderSize = ethers.parseUnits("3000", 6);     // $3000 notional in USDC precision (10x on $300)
        const entryPrice = ethers.parseUnits("3000", 18);   // $3000 in 18-dec
        const collateral = ethers.parseUnits("300", 18);    // $300 in 18-dec internal precision
        ...
        await env.trading.connect(env.alice).createOrder(
            1, // MARKET_DECREASE
            marketDetails.collection,
            halfSize, // half of stored position size
            0, 0, true, 10000, 1,
            { value: ethers.parseEther("0.01") }
        );
    ...
    it("should handle Dividend Settlement when Corporate Action is triggered", async function () {
        const orderSize = ethers.parseUnits("3000", 6);  // $3000 notional
        const entryPrice = ethers.parseUnits("3000", 18);
        const collateral = ethers.parseUnits("300", 18);
