...
describe("Branch Coverage 3 - Trading and Liquidation", function () {
    let env: any;
    ...
    async function setupMarketAndDeposit(aliceAmount: bigint = 100000n * 10n**6n) {
        ...
        await env.trading.connect(env.admin).setMarket(
            market, market, 100, ethers.parseEther("1000000"), ethers.parseEther("10000000"), 500, 1000, 86400
        );
    ...
    describe("TradingCore: Collateral Management", function () {
        it("addCollateral and withdrawCollateral successfully", async function () {
            ...
            await env.trading.connect(env.alice).createOrder(
                0, market, ethers.parseUnits("100", 6), ethers.parseEther("20"), 0, true, 500, 0, { value: ethers.parseEther("0.01") }
            );
        ...
        it("addCollateral max leverage reverting", async function () {
            const { market } = await setupMarketAndDeposit();
            await env.trading.connect(env.alice).createOrder(
                0, market, ethers.parseUnits("2000", 6), ethers.parseEther("100"), 0, true, 500, 0, { value: ethers.parseEther("0.01") }
            );
            await env.trading.connect(env.keeper).executeOrder(1, []);
        ...
        it("withdrawCollateral reverting on insufficient collateral", async function () {
            const { market } = await setupMarketAndDeposit();
            await env.trading.connect(env.alice).createOrder(
                0, market, ethers.parseUnits("1000", 6), ethers.parseEther("200"), 0, true, 500, 0, { value: ethers.parseEther("0.01") }
            );
            await env.trading.connect(env.keeper).executeOrder(1, []);
    ...
    describe("TradingCore: Liquidation paths", function () {
        it("liquidatePosition branches", async function () {
            ...
            await env.trading.connect(env.alice).createOrder(
                0, market, ethers.parseUnits("1000", 6), ethers.parseEther("20"), 0, true, 500, 0, { value: ethers.parseEther("0.01") }
            );
            await env.trading.connect(env.keeper).executeOrder(1, []);
        ...
        it("liquidatePosition: not liquidatable revert", async function () {
            const { market } = await setupMarketAndDeposit();
            await env.trading.connect(env.alice).createOrder(
                0, market, ethers.parseUnits("100", 6), ethers.parseEther("50"), 0, true, 500, 0, { value: ethers.parseEther("0.01") }
            );
            await env.trading.connect(env.keeper).executeOrder(1, []);
    ...
    describe("TradingCore: Complex Orders (Decrease, Limit)", function () {
        it("MARKET_DECREASE order successfully", async function () {
            ...
            // Note: sizeDelta for decrease is NOT scaled via toInternalPrecision in TradingLib,
            // so we should pass it in 1e18 if that's what's expected for position.size subtraction.
            await env.trading.connect(env.alice).createOrder(
                1, market, ethers.parseEther("50"), 0, 0, true, 500, 1, { value: ethers.parseEther("0.01") }
            );
            await env.trading.connect(env.keeper).executeOrder(2, []);
            
            const pos = await env.trading.getPosition(1);
            expect(pos.size).to.equal(ethers.parseEther("50"));
        });
        ...
        it("LIMIT_INCREASE executed", async function () {
            ...
            // triggerPrice = 45000
            await env.trading.connect(env.alice).createOrder(
                2, market, ethers.parseUnits("100", 6), ethers.parseEther("20"), ethers.parseEther("45000"), true, 500, 0, { value: ethers.parseEther("0.01") }
            );
