...
let cachedAt = 0;
let cachedPrices = {};
function parsePythPrice(priceStr, expo) {
...
export async function fetchPyth24hChange(marketAddress) {
    ...
    if (!current || current <= 0 || !history.length)
        return undefined;
...
export async function fetchPythPriceHistoryHermes(marketAddress, days = 7, points = 24) {
    ...
    for (let i = points - 1; i >= 0; i--) {
        ...
        try {
            ...
            if (value > 0)
                pricePoints.push({ timestamp: ts * 1000, value });
        }
        catch {
            /* skip failed point */
        }
