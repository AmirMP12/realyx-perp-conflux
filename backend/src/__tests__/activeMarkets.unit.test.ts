...
describe("Active Markets Logic Paths", () => {
    ...
    it("returns no addresses when the active-markets filter is disabled", async () => {
        process.env.ENABLE_ACTIVE_MARKETS_FILTER = "false";
        expect(await activeMarkets.getActiveMarketAddresses()).toBeNull();
    });

    it("returns null when the trading core address is missing and caches resolved addresses", async () => {
    ...
    it("returns null when every RPC endpoint fails", async () => {
