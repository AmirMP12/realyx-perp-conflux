...
describe("Comprehensive System Logic Paths", () => {
    ...
    describe("sync service and routes", () => {
        it("handles runSync edge cases", async () => {
        ...
        it("serves stats when market addresses are non-string and TVL fetch fails", async () => {
        ...
        it("reports detailed health when a service returns null", async () => {
    ...
    describe("indexer and stats logic", () => {
        it("computes protocol metrics for day and hour periods", async () => {
            mockPoolQuery.mockResolvedValue({ rows: [] });
            await indexer.fetchProtocolMetrics(10, "day");
            await indexer.fetchProtocolMetrics(10, "hour");
        });

        it("caches the stats response across repeated calls", async () => {
            const statsModule = await import("../routes/stats.js");
            const statsApp = express(); statsApp.use("/", statsModule.default);
