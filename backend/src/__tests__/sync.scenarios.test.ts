...
describe("Sync Route Exhaustive Scenarios", () => {
    ...
    it("should process PositionClosed and PositionLiquidated events", async () => {
        ...
        // Call 2: PositionClosed — resolved from the in-batch PositionOpened
        // (same position id) via the within-pulse cache, which is authoritative
        // and avoids an N+1 DB lookup. The open carried account "0xtrader".
        expect(insertCalls[1][1]).toContain("0xtrader");
        expect(insertCalls[1][1]).toContain("PositionClosed");

        // Call 3: PositionLiquidated — likewise resolved to the opener "0xtrader"
        // from cache (on-chain the log only carries the liquidator).
        expect(insertCalls[2][1]).toContain("0xtrader");
