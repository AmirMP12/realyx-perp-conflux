...
describe("Global System Integration Logic", () => {
    ...
    describe("index.ts bootstrap and intervals", () => {
        beforeEach(() => {
            jest.resetModules();
        });

        it("bootstraps with WebSockets enabled and schedules the sync interval", async () => {
        ...
        it("skips the sync interval when DISABLE_INBAND_SYNC=true", async () => {
             const { bootstrap } = await import("../index.js");
    ...
    describe("health.ts extra scenarios", () => {
        it("returns 503 and handles non-error rejections", async () => {
    ...
    describe("user.ts resolveMarketSymbol and trade history", () => {
        ...
        it("handles user route error paths", async () => {
             const userModule = await import("../routes/user.js");
             const userRouter = userModule.default;
             const app = express(); app.use("/user", userRouter);
    ...
    describe("index.ts top-level catch", () => {
        it("logs and surfaces errors thrown during bootstrap", async () => {
        ...
        it("triggers top-level catch in index.js", async () => {
             // Force the top-level catch to run
    ...
    describe("Markets Route Logic Paths", () => {
        it("handles error paths in the markets route", async () => {
             ...
             // 2. Reject paths to trigger the catch handlers
             // We need fetchMarkets to return an empty array to enter the fallback block
             ...
             // These don't have an inline catch here, but the fallback map does:
             ...
             await request(app).get("/"); // Triggers the fallback catch

             // Now run the active-markets success block
    ...
    describe("coingecko.ts mapping and utility", () => {
        it("maps coingecko ids for markets and price history", async () => {
             jest.resetModules();
